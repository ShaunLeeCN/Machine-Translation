...
class Attention(nn.Module):
    ...
    def forward(
        ...
        attention_scores = torch.matmul(xq, xk.transpose(-2, -1)) / math.sqrt(self.head_dim)
        causal_mask = self.mask[:, :, :seq_len, :seq_len]
        attention_scores = attention_scores + causal_mask
        attention_probs = F.softmax(attention_scores, dim=-1)
        attention_probs = self.attn_dropout(attention_probs)
        attention_output = torch.matmul(attention_probs, xv)
        attention_output = attention_output.transpose(1, 2).contiguous()
        attention_output = attention_output.reshape(bsz, seq_len, -1)
        output = self.resid_dropout(self.wo(attention_output))
