...
def dpo_loss(ref_probs: torch.Tensor, probs: torch.Tensor, mask: torch.Tensor, beta: float = 0.1) -> torch.Tensor:
    ...
    # Calculate loss
    chosen_log_probs = chosen_probs
    rejected_log_probs = reject_probs

    chosen_ref_log_probs = chosen_ref_probs
    rejected_ref_log_probs = reject_ref_probs

    pi_logratios = chosen_log_probs - chosen_ref_log_probs
    ref_logratios = rejected_log_probs - rejected_ref_log_probs

    logits = pi_logratios - ref_logratios
    loss = -F.logsigmoid(beta * torch.clamp(logits, min=-50, max=50))
